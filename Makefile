...
serverledge:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o $(BIN)/$@ cmd/$@/main.go

lb:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o $(BIN)/$@ cmd/$@/main.go

serverledge-cli:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o $(BIN)/$@ cmd/cli/main.go

executor:
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o $(BIN)/$@ cmd/$@/executor.go
